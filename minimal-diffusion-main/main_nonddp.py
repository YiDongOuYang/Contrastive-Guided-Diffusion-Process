...
# import contrastive.contrastive_loss as contrast_loss
import contrastive.contrastive_loss_mnist as contrast_loss
    ...
    def sample_from_reverse_process(
        ...
        for i, t in zip(np.arange(timesteps)[::-1], new_timesteps[::-1]):
            with torch.no_grad():
                ...
                if (t<150)&(t>50):#recheck it 
                    delta= contrast_loss.get_Acquisition_grad(final,final_last)
...
def sample_N_images(
    ...
    with tqdm(total=math.ceil(N / (args.batch_size * num_processes))) as pbar:
        while num_samples < N:
            ...
            samples_list = [gen_images for _ in range(num_processes)]
            if args.class_cond:
                # labels_list = [torch.zeros_like(y) for _ in range(num_processes)]
                labels_list = [y for _ in range(num_processes)]
...
def main():
    ...
    if args.sampling_only:
        ...
        cv2.imwrite(
                    os.path.join(
                        args.save_dir,
                        f"{args.arch}_{args.dataset}-{args.diffusion_steps}_steps-{args.sampling_steps}-sampling_steps-class_condn_{args.class_cond}_5000.png",
                    ),
                    np.concatenate(sampled_images, axis=1)[:, :, ::-1],
                )
        np.savez(
            os.path.join(
                args.save_dir,'4000_mnist',
                f"{args.arch}_{args.dataset}-{args.sampling_steps}-sampling_steps-{len(sampled_images)}_images-class_condn_{args.class_cond}_100k.npz",
            ),
            sampled_images,
            labels,
        )
    ...
    #training only on a subset
    # ipdb.set_trace()
    train_set = torch.utils.data.Subset(train_set, np.arange(0, 4000))  
        ...
        if not epoch % 1:
            ...
            if args.local_rank == 0:
                cv2.imwrite(
                    os.path.join(
                        args.save_dir,
                        f"{args.arch}_{args.dataset}-{args.diffusion_steps}_steps-{args.sampling_steps}-sampling_steps-class_condn_{args.class_cond}_4000.png",
                    ),
                    np.concatenate(sampled_images, axis=1)[:, :, ::-1],
                )
        if args.local_rank == 0:
            torch.save(
                ...
                os.path.join(
                    args.save_dir,
                    f"{args.arch}_{args.dataset}-epoch_{args.epochs}-timesteps_{args.diffusion_steps}-class_condn_{args.class_cond}_4000.pt",
                ),
            )
            torch.save(
                args.ema_dict,
                os.path.join(
                    args.save_dir,
                    f"{args.arch}_{args.dataset}-epoch_{args.epochs}-timesteps_{args.diffusion_steps}-class_condn_{args.class_cond}_ema_{args.ema_w}_4000.pt",
                ),
            )
