        ...
        if self.train:
            ...
            if aux_data_filename is not None:
                ...
                if os.path.splitext(aux_path)[1] == '.pickle':
                    ...
                    aux_data = aux['arr_0']
                    aux_targets = aux['arr_1']
                    aux_data_torch = []
                    # ipdb.set_trace()
                    for i in range(aux_data.shape[0]):
                        aux_data_torch.append(self.dataset.transform(Image.fromarray(aux_data[i].squeeze(), mode="L")))
                    aux_data = torch.stack(aux_data_torch)
                ...
                #very important for mnist, to normalize
                # ipdb.set_trace()
                # self.data = self.data.float().div(255)
                aux_data = (aux_data*255).byte()

                # self.data = torch.cat((self.data, aux_data.squeeze()), axis=0)
                # # ipdb.set_trace()
                ...
                #modification for loading subset of data
                
                self.sup_indices = list(range(4000))
                self.data = torch.cat((self.data[:4000], aux_data.squeeze()[:1000,]), axis=0)
                # ipdb.set_trace()
                if not add_aux_labels:
                    self.targets.extend([-1] * len(aux_data))
                else:
                    self.targets = self.targets.tolist()[:4000]
                    self.targets.extend(aux_targets[:1000])
                self.unsup_indices.extend(range(4000, 5000))
                # ipdb.set_trace()
                # self.unsup_indices.extend(range(orig_len, orig_len+len(aux_data)))
